...
const pizza = {
    ...
    popularity: 2000,
    rating: 7,
    tags: ['gluten-free', 'kids', 'small', 'medium', 'large', 'extra large']
...
const filteredFood = foodArr.filter((tag) => tag.tags.includes('vegan'));
...
function filterByProperty(property, number)
{
    ...
    //     if(number < element[property])
    //     {
    //         filterArr.push(element);
    //     }
    // });
    const filterArr = foodArr.filter((val) => val[property] > number);
...
console.log(filterByProperty('price', 17));